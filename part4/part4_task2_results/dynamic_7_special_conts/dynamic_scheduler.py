...
HIGH_THRESHOLD_SINGLE_CORE = 85.0  # When to scale up memcached cores
LOW_THRESHOLD_SINGLE_CORE = 60.0   # When to scale down memcached cores
HIGH_THRESHOLD_TWO_CORES = 75.0  # When to scale up memcached cores
LOW_THRESHOLD_TWO_CORES = 60.0   # When to scale down memcached cores
...
def main():
    ...
    try:
        ...
        # Start first regular job on cores 1,2,3
        regular_job = job_queue.pop(0)
        log_message(f"Starting regular job: {regular_job} with 4 threads on cores {batch_cores}")
        regular_container = run_batch_job(regular_job, batch_cores, 4)
    
        timer.start(regular_job)
        logger.job_start(Job(regular_job), batch_cores, 4)
        running_jobs.append((regular_job, regular_container, batch_cores, 4))
        ...
        while running_jobs or job_queue or special_job_queue:
            ...
            if current_state == MEMCACHED_ONLY_CORE0 and core0_usage > HIGH_THRESHOLD_SINGLE_CORE:
                ...
                # Move jobs back to cores 1,2,3 (but not special jobs)
                for i, (job_name, container, job_cores, threads) in enumerate(running_jobs):
                    # Don't move special jobs (freqmine/ferret)
                    if job_name in SPECIAL_JOBS:
                        continue
                ...
                if not is_container_running(container):
                    ...
                    if job_name in SPECIAL_JOBS:
                        if len(special_job_queue) > 0:  # If ferret is still in queue
                            ...
                            # Both special jobs done, start a regular job on these cores if available
                            if job_queue:
                                next_job = job_queue.pop(0)
                                log_message(f"Starting regular job on special cores: {next_job}")
                                next_container = run_batch_job(next_job, special_job_cores, 4)
                                
                                timer.start(next_job)
                                logger.job_start(Job(next_job), special_job_cores, 4)
                                running_jobs.append((next_job, next_container, special_job_cores, 4))
                                log_message(f"Job {next_job} started successfully")
                    else:
                        if job_queue:
                            next_job = job_queue.pop(0)
                            log_message(f"Starting next regular job: {next_job}")
                            next_container = run_batch_job(next_job, job_cores, 4)
                            
                            timer.start(next_job)
                            logger.job_start(Job(next_job), job_cores, 4)
                            running_jobs.append((next_job, next_container, job_cores, 4))
                            log_message(f"Job {next_job} started successfully")
                else:
                    i += 1
