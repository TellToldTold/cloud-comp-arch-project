...
def setup_mcperf_agents(force_install = False):
    """
    Discovers client-agent and client-measure nodes, sets up and builds mcperf
    on each. If mcperf is already installed, it skips installation.
    
    This function:
    1) Uses `kubectl get nodes -o json` to find nodes named client-agent and
    client-measure.
