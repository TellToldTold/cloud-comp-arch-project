...
HIGH_THRESHOLD_CORE_ZERO = 85.0    # Moving jobs off core 0
LOW_THRESHOLD_CORE_ZERO = 40.0     # Moving jobs back to core 0

# Colocation states
MEMCACHED_CORE0_JOBS_ALL = "memcached_core0_jobs_all"      # Memcached on core 0, containers on 0,1,2,3
MEMCACHED_CORE0_JOBS_123 = "memcached_core0_jobs_123"      # Memcached on core 0, containers on 1,2,3
MEMCACHED_CORES01_JOBS_123 = "memcached_cores01_jobs_123"  # Memcached on cores 0,1, containers on 1,2,3
MEMCACHED_CORES01_JOBS_23 = "memcached_cores01_jobs_23"    # Memcached on cores 0,1, containers on 2,3
...
def get_memcached_core0_usage():
    """
    Get the CPU usage of memcached thread running on core 0.
    
    Returns:
        float: CPU usage percentage of memcached on core 0, or 0 if not found
    """
    thread_usage = get_memcached_cpu_percent()
    for tid, (cpu_percent, core) in thread_usage.items():
        if core == 0:  # Thread running on core 0
            return cpu_percent
    return 0.0  # Return 0 if no thread found on core 0
...
def main():
    ...
    try:
        ...
        while running_jobs:
            # Get CPU usage based on current state
            if current_state == MEMCACHED_CORE0_JOBS_123 or current_state == MEMCACHED_CORES01_JOBS_23:
                # When memcached is alone on core 0, we can use overall core usage
                cpu_usage = get_cpu_usage_per_core()
                core0_usage = cpu_usage[0]
                log_message(f"Core 0 usage: {core0_usage:.1f}%")
            else:
                # When memcached is colocated with jobs on core 0, we need to check its thread usage
                core0_usage = get_memcached_core0_usage()
                log_message(f"Memcached thread on core 0 usage: {core0_usage:.1f}%")
            
            # Check if we need to adjust core allocation based on current state
            if current_state == MEMCACHED_CORE0_JOBS_123:
                # Check if memcached needs more cores
                if core0_usage > HIGH_THRESHOLD_SINGLE_CORE:
                    log_message("High memcached usage detected, scaling up to cores 0,1")
                    memcached_cores = [0, 1]
                    set_memcached_affinity(memcached_cores)
                    
                    # Update state
                    current_state = MEMCACHED_CORES01_JOBS_123
                    
                    # Log the change
                    logger.update_cores(Job.MEMCACHED, memcached_cores)
                    logger.custom_event(Job.MEMCACHED, "expanded_to_core1")
                
                # Check if we can use core 0 for batch jobs
                elif core0_usage < LOW_THRESHOLD_CORE_ZERO and running_jobs and not jobs_on_core0:
                    log_message("Low memcached usage detected, allowing batch job on core 0")
                    
                    # Move one job to use core 0
                    job_name, container, job_cores, threads = running_jobs[0]
                    new_cores = [0, 1, 2, 3]
                    update_container_cores(container, new_cores)
                    running_jobs[0] = (job_name, container, new_cores, threads)
                    
                    # Log the change
                    logger.update_cores(Job(job_name), new_cores)
                    logger.custom_event(Job(job_name), "expanded_to_core0")
                    
                    # Track job using core 0
                    jobs_on_core0.append(job_name)
                    
                    # Update state
                    current_state = MEMCACHED_CORE0_JOBS_ALL
            
            elif current_state == MEMCACHED_CORE0_JOBS_ALL:
                # When memcached and jobs share core 0
                if core0_usage > HIGH_THRESHOLD_CORE_ZERO:
                    # Move jobs off core 0 one at a time
                    for i, (job_name, container, job_cores, threads) in enumerate(running_jobs):
                        if job_name in jobs_on_core0:
                            log_message(f"Moving job {job_name} off core 0")
                            new_cores = [1, 2, 3]
                            update_container_cores(container, new_cores)
                            ...
                            # Log the change
                            logger.update_cores(Job(job_name), new_cores)
                            logger.custom_event(Job(job_name), "moved_off_core0")
                            
                            # Remove from tracking
                            jobs_on_core0.remove(job_name)
                            
                            # Update state if all jobs moved off core 0
                            if not jobs_on_core0:
                                current_state = MEMCACHED_CORE0_JOBS_123
                            
                            # Only move one job at a time
                            break
            
            elif current_state == MEMCACHED_CORES01_JOBS_123:
                if core0_usage < LOW_THRESHOLD_SINGLE_CORE:
                    ...
                    # Log the change
                    logger.update_cores(Job.MEMCACHED, memcached_cores)
                    logger.custom_event(Job.MEMCACHED, "reduced_to_core0")
                
                elif core0_usage > HIGH_THRESHOLD_TWO_CORES:
                    # Move jobs off core 1 one at a time
                    for i, (job_name, container, job_cores, threads) in enumerate(running_jobs):
                        if 1 in job_cores and job_name not in jobs_on_core1:
                            ...
                            jobs_on_core1.append(job_name)
                            
                            # Update state if all jobs moved off core 1
                            if len(jobs_on_core1) == len(running_jobs):
                                current_state = MEMCACHED_CORES01_JOBS_23
                            
                            # Only move one job at a time
                            break
            
            elif current_state == MEMCACHED_CORES01_JOBS_23:
                if core0_usage < LOW_THRESHOLD_TWO_CORES:
                    # Allow jobs back on core 1 one at a time
                    if jobs_on_core1 and running_jobs:
                        job_to_move = jobs_on_core1.pop(0)  # Get first job that was moved off core 1
                        ...
                        for i, (job_name, container, job_cores, threads) in enumerate(running_jobs):
                            if job_name == job_to_move:
                                ...
                                # Update state if any job is now on core 1
                                current_state = MEMCACHED_CORES01_JOBS_123
                                break
                
                elif core0_usage < LOW_THRESHOLD_SINGLE_CORE:
                    # Scale down memcached to only core 0
                    log_message("Very low memcached usage detected, scaling down to core 0 only")
                    memcached_cores = [0]
                    set_memcached_affinity(memcached_cores)
                    
                    # Update state - jobs still on cores 2,3
                    current_state = MEMCACHED_CORE0_JOBS_123
                    ...
                    # Allow jobs back on core 1 one at a time
                    if jobs_on_core1 and running_jobs:
                        job_to_move = jobs_on_core1.pop(0)
            ...
            while i < len(running_jobs):
                ...
                if not is_container_running(container):
                    ...
                    # Remove from tracking lists if it's there
                    if job_name in jobs_on_core0:
                        jobs_on_core0.remove(job_name)
                    if job_name in jobs_on_core1:
                        jobs_on_core1.remove(job_name)
