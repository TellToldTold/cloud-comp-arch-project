...
from memcached_manager import (
    ...
    get_memcached_cpu_percent_per_core
)
...
def main():
    ...
    try:
        ...
        # Get initial memcached CPU affinity
        memcached_cores = get_memcached_cpu_affinity()
        print(memcached_cores)
        logger.job_start(Job.MEMCACHED, memcached_cores, len(memcached_cores))
        
        # Log initial CPU usage
        memcached_cpu_percent = get_memcached_cpu_percent()
        print(memcached_cpu_percent)
        logger.custom_event(Job.SCHEDULER, f"Initial memcached CPU usage: {memcached_cpu_percent:.2f}%")
        
        # Log per-core usage
        per_core_usage = get_memcached_cpu_percent_per_core()
        print(per_core_usage)
        logger.custom_event(Job.SCHEDULER, f"Initial memcached per-core usage: {per_core_usage}")
